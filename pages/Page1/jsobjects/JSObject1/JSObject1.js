export default {
	...
	async trustMe() {
		...
		} else if (assessment_section_selected.trim() === "Pool_config_&_No_Section") {
			dynamic_key = {
		...
		} else if (assessment_section_selected.trim() === "Section_Data") {
			dynamic_key = {
				sectionList: assessment_section_sectionList.Sections.map((section) => ({
					skillIds: section.skillIds.split(",").map(Number),
					id: section.id || null,
					...
					queryList: section.queryList.map((query) => ({
						...
						questionType: query.questionType || "SINGLE_SELECT_MCQ",
					...
					instruction: {
						...
						instructionInfoList: section.instruction.instructionInfoList.map((info) => ({
							...
							instructionList: info.instructionList.split(",").map((item) => item.trim()),
						})),
					},
				})),
				enableSection: true,
